"""The door's configuration: one TOML file, a few keys.
    ...
    tools_host = "tools.yunhan.me"      # optional; defaults to tools.<site>
    ...
    home = "~/Projects/yunhan.me/home"  # public landing assets and private Toolbox
...
class Config:
    ...
    site: str = "yunhan.me"
    tools_host: str = ""
    """Tool hostname; empty derives tools.<site> (same-origin on loopback previews)."""
    cookie_name: str = "yh_session"
    cookie_domain: str = ""
    cookie_days: int = 30
    origins: tuple[str, ...] = ()
    instrument: Path = field(default_factory=lambda: Path(__file__).resolve().parents[2] / "instrument")
    home: Path = field(default_factory=lambda: Path(__file__).resolve().parents[2] / "home")
    """Homepage tree: public/ is static; private/toolbox.html requires a session."""
...
def load(path: Path | None = None) -> Config:
    ...
    try:
        ...
        return cfg
    for key in ("bind", "site", "tools_host", "cookie_name", "cookie_domain"):
        if key in raw:
