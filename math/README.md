...
Deployment is forwarded to the infrastructure repository by `scripts/push.sh` from the repo root. It previews by default; use `scripts/push.sh --apply` to deploy. Files are read from disk per request, so they are live when they land.
